...
let upload_web_url = window.location.protocol + "//upload.yunzhongzhuan.com";

let api_upload_web_url;
// 初始化上传地址的 iframe
function api_upload_web_url_load(){
	api_upload_web_url = upload_web_url + "/v10/upload";
}
...
function showpay(){
	...
	content_element.innerHTML = '<img src="//global.yunzhongzhuan.com/img/wechat_alipay.gif" />';
	swal({
...
function show_verify_code(){
	...
	if( false && navigator.language.toLowerCase().indexOf('cn')==-1){
		...
		new_src = new_src.replace(api_server_url,window.location.protocol+'//apiyunzhongzhuancom.vercel.app');
			...
			if(verify_pass_input_items.length>0){
				...
				if(verify_pass_input_value.length>0&&verify_pass_input_value.length==12){
					...
					if(navigator.language.toLowerCase().indexOf('cn')==-1){
						...
						xmlhttp.open("GET",window.location.protocol+"//apiyunzhongzhuancom.vercel.app/php/v4/verify_code?session_id="+userinfo["session_id"]+"&submit_verify_code="+verify_pass_input_value+"&t="+new Date().getTime(),true);
					}else{
	...
	if(verify_pass_img_items.length>0){
		...
		verify_pass_img.onclick = function(){
			if( false && navigator.language.toLowerCase().indexOf('cn')==-1){
				...
				this.src = window.location.protocol+'//apiyunzhongzhuancom.vercel.app/php/v4/verify_code?session_id=' + userinfo["session_id"] + '&t=' + new Date().getTime();
			}else{
